...
function placeFlower(region, worldX, worldY, worldZ, blockId) {
  if (!canReplaceDecorationBlock(getRegionBlock(region, worldX, worldY, worldZ))) {
    return false;
  }
...
function placeMushroom(region, worldX, worldY, worldZ, blockId) {
  ...
  if (!isSolidRenderBlock(getRegionBlock(region, worldX, worldY - 1, worldZ))) {
    return false;
  }
...
function canPlaceCactus(region, worldX, worldY, worldZ) {
  ...
  return countHorizontalNeighbors(region, worldX, worldY, worldZ, (blockId) => isSolid(blockId)) === 0;
}
...
function placePlantFeature(region, random, worldX, worldY, worldZ, attempts, callback) {
...
function placeFlowerFeature(region, random, worldX, worldY, worldZ, blockId) {
  return placePlantFeature(region, random, worldX, worldY, worldZ, 64, (x, y, z) => {
...
function placeTallGrassFeature(region, random, worldX, worldY, worldZ) {
  ...
  return placePlantFeature(region, random, worldX, baseY, worldZ, 128, (x, y, z) => {
...
function placeDeadBushFeature(region, random, worldX, worldY, worldZ) {
  const baseY = descendFeatureBaseY(region, worldX, worldY, worldZ);
  return placePlantFeature(region, random, worldX, baseY, worldZ, 4, (x, y, z) => {
...
export function createFloraSession(region, sourceBiome, random, chunkX, chunkZ) {
  return {
    region,
    sourceBiome,
    random,
...
export function advanceFloraSession(session) {
  ...
  switch (session.stage) {
    ...
    case "brown_mushroom":
      if (session.random.nextInt(4) === 0) {
        placePlantFeature(
          ...
          nextFeatureZ(session.random, session.chunkZ),
          64,
          (x, y, z) => placeMushroom(session.region, x, y, z, EXTRA_BLOCKS.BROWN_MUSHROOM),
        );
      }
    ...
    case "red_mushroom":
      if (session.random.nextInt(8) === 0) {
        placePlantFeature(
          ...
          nextFeatureZ(session.random, session.chunkZ),
          64,
          (x, y, z) => placeMushroom(session.region, x, y, z, EXTRA_BLOCKS.RED_MUSHROOM),
        );
      }
    ...
    case "tall_grass":
      ...
      session.stage = session.sourceBiome === BIOME_IDS.DESERT ? "dead_bush" : "sugar_cane";
      session.stageAttempt = 0;
      return true;
    ...
    case "pumpkin":
      ...
      session.stage = session.sourceBiome === BIOME_IDS.DESERT ? "cactus" : "complete";
      session.stageAttempt = 0;
...
export function applyFlora(region, sourceBiome, random, chunkX, chunkZ) {
  const session = createFloraSession(region, sourceBiome, random, chunkX, chunkZ);
