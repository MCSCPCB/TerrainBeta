...
const ORE_CONFIG = Object.freeze([
  { block: BLOCKS.DIRT, attempts: 20, size: 32, distribution: "linear", minY: 0, maxY: 128 },
  { block: BLOCKS.GRAVEL, attempts: 10, size: 32, distribution: "linear", minY: 0, maxY: 128 },
  { block: EXTRA_BLOCKS.COAL_ORE, attempts: 20, size: 16, distribution: "linear", minY: 0, maxY: 128 },
  { block: EXTRA_BLOCKS.IRON_ORE, attempts: 20, size: 8, distribution: "linear", minY: 0, maxY: 64 },
  { block: EXTRA_BLOCKS.GOLD_ORE, attempts: 2, size: 8, distribution: "linear", minY: 0, maxY: 32 },
  { block: EXTRA_BLOCKS.REDSTONE_ORE, attempts: 8, size: 7, distribution: "linear", minY: 0, maxY: 16 },
  { block: EXTRA_BLOCKS.DIAMOND_ORE, attempts: 1, size: 7, distribution: "linear", minY: 0, maxY: 16 },
  { block: EXTRA_BLOCKS.LAPIS_ORE, attempts: 1, size: 6, distribution: "centered", centerY: 16, radiusY: 16 },
]);
...
function generateReplaceableVein(region, random, replaceableBlocks, oreBlock, size, baseX, baseY, baseZ) {
  ...
  for (let step = 0; step <= size; step += 1) {
    ...
    const diameter = (Math.sin((step * Math.PI) / size) + 1.0) * radiusScale + 1.0;
    const radius = diameter / 2.0;
    const minX = Math.max(Math.floor(centerX - radius), region.originWorldX);
    const maxX = Math.min(
    ...
    const minZ = Math.max(Math.floor(centerZ - radius), region.originWorldZ);
    const maxZ = Math.min(
    ...
    for (let x = minX; x <= maxX; x += 1) {
      ...
      for (let y = minY; y <= maxY; y += 1) {
        ...
        for (let z = minZ; z <= maxZ; z += 1) {
          ...
          if (replaceableBlocks.has(getRegionBlock(region, x, y, z))) {
...
function pickOreY(random, config) {
  if (config.distribution === "centered") {
    return random.nextInt(config.radiusY) + random.nextInt(config.radiusY) + config.centerY - config.radiusY;
  }
...
export function advanceClaySession(session, maxAttempts = 1) {
  ...
  while (processed < maxAttempts && session.attempt < CLAY_CONFIG.attempts) {
    ...
    const y = session.random.nextInt(CLAY_CONFIG.maxY - CLAY_CONFIG.minY) + CLAY_CONFIG.minY;
