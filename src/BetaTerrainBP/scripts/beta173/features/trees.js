...
function selectTreeType(sourceBiome, random) {
  switch (sourceBiome) {
    case BIOME_IDS.TAIGA:
      return random.nextInt(3) === 0 ? "pine" : "spruce";
    case BIOME_IDS.FOREST:
      if (random.nextInt(5) === 0) {
        return "birch";
      }
      return random.nextInt(3) === 0 ? "big_oak" : "oak";
    case BIOME_IDS.RAINFOREST:
      return random.nextInt(3) === 0 ? "big_oak" : "oak";
    default:
      return random.nextInt(10) === 0 ? "big_oak" : "oak";
  }
}

function placeSelectedTree(region, random, sourceBiome, x, y, z) {
  const treeType = selectTreeType(sourceBiome, random);
...
function getTreeCount(treeNoise, random, biomeId, chunkX, chunkZ) {
  const blockX = chunkX * 16;
  const blockZ = chunkZ * 16;
  const noise = treeNoise.generate(
    null,
    blockX * 0.5,
    blockZ * 0.5,
    1,
  ...
  const forestNoise = Math.trunc((noise / 8.0 + random.nextDouble() * 4.0 + 4.0) / 3.0);
  let count = 0;
  if (random.nextInt(10) === 0) {
    count += 1;
  }

  if (biomeId === BIOME_IDS.FOREST || biomeId === BIOME_IDS.RAINFOREST || biomeId === BIOME_IDS.TAIGA) {
    count += forestNoise + 5;
  } else if (biomeId === BIOME_IDS.SEASONAL_FOREST) {
    count += forestNoise + 2;
  } else if (biomeId === BIOME_IDS.DESERT || biomeId === BIOME_IDS.TUNDRA || biomeId === BIOME_IDS.PLAINS) {
    count -= 20;
  }
...
export function applyTrees(region, sourceBiome, treeNoise, random, chunkX, chunkZ) {
  const session = createTreeSession(region, sourceBiome, treeNoise, random, chunkX, chunkZ);
  while (advanceTreeSession(session)) {
    // Run the exact same deterministic state machine to completion for synchronous callers.
  }
}

export function createTreeSession(region, sourceBiome, treeNoise, random, chunkX, chunkZ) {
  return {
    region,
    sourceBiome,
    treeNoise,
...
export function advanceTreeSession(session, maxAttempts = 1) {
  ...
  if (session.treeCount === null) {
    session.treeCount = getTreeCount(
      session.treeNoise,
      session.random,
      session.sourceBiome,
      session.chunkX,
  ...
  while (processed < maxAttempts && session.attempt < session.treeCount) {
    ...
    if (baseY >= 1 && baseY < 128) {
      placeSelectedTree(session.region, session.random, session.sourceBiome, x, baseY, z);
    }
