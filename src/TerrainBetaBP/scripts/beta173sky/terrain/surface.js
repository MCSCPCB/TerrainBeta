...
function replaceSurfaceColumn(session, x, z) {
     const { blocks, climateData, depthBuffer, random, surfacePalette } =
          session;
     const biomeId = climateData.biomes[layerIndex(x, z)];
     const surface = getSurfaceForBiome(biomeId, surfacePalette);
     let surfaceDepth = Math.trunc(
          depthBuffer[layerIndex(x, z)] / 3.0 +
               3.0 +
...
export function createSurfaceReplacementSession(
     ...
     chunkX,
     chunkZ,
     surfacePalette = generator.surfacePalette ?? null,
) {
...
export function replaceBlocksForBiomes(
     blocks,
     generator,
     climateData,
     chunkX,
