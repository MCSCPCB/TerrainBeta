...
function buildSubmittedGeneratorConfig(formValues) {
     ...
     const visibleOptions =
          GENERATOR_VISIBLE_OPTIONS[generatorKey] ?? new Set();
...
function closePlayerForms(player) {
     warnOnError("Failed to close forms", () =>
     ...
     try {
          ...
          if (keepLoadingFormOpen) {
               const elapsedMs = Math.max(
                    0,
                    Date.now() - loadingFormOpenedAtMs,
               );
...
function isPlayerAtInitializationLoadingHeight(player) {
     return (
          Math.abs(player.location.y - PRE_ACTIVATION_PLAYER_HOLD_HEIGHT) <= 1
     );
}
...
function savePreActivationHoldState(player) {
     ...
     setPreActivationHoldState(player, {
          x: player.location.x,
          z: player.location.z,
...
function ensurePreActivationPlayerSafety(player) {
     ...
     const playerChunk = playerIsInTargetDimension
          ? getChunkForLocation(player.location)
          : null;
     if (
          ...
          while (isPlayerValid(player) && !isWorldGenerationActivated()) {
               ...
               await showTimedLoadingForm(
                    ...
                    () => {
                         ...
                         saveStoredWorldgenConfigSubmission(
                              submittedGeneratorConfig,
                         );
                         promotePreActivationHoldStateToInitializationReturnState(
                              player,
                         );
                         ...
                         const generationStarted =
                              startConfiguredWorldGeneration(player);
