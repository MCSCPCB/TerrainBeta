...
function spawnAnchor(player, renderId, targetLocation) {
     const anchor = player.dimension.spawnEntity(
          ANCHOR_TYPE_ID,
          targetLocation,
     );
...
function ensureAnchorForPlayer(player, anchorsByRenderId) {
     ...
     if (!anchor?.isValid()) {
          try {
               ...
               console.warn(
                    `Failed to spawn alpha fog anchor for ${player.name}: ${error}`,
               );
               return undefined;
          }
     } else if (
          getDistanceSquared(anchor.location, targetLocation) >
          ...
          if (!moved) {
               try {
                    ...
                    console.warn(
                         `Failed to move alpha fog anchor for ${player.name}: ${error}`,
                    );
                    return undefined;
               }
          }
...
function despawnAnchor(anchor) {
     try {
          ...
          console.warn(
               `Failed to despawn stale alpha fog anchor ${anchor.id}: ${error}`,
          );
     }
}
...
function sweepDimension(dimensionId) {
     ...
     try {
          ...
          console.warn(
               `Failed to access dimension ${dimensionId} for alpha fog: ${error}`,
          );
          return;
     }
