...
function countOpenDungeonSides(
     region,
     worldX,
     worldY,
     worldZ,
...
function generateDungeon(region, random, worldX, worldY, worldZ) {
     ...
     const openings = countOpenDungeonSides(
          region,
          worldX,
          worldY,
          worldZ,
     ...
     for (let x = worldX - radiusX - 1; x <= worldX + radiusX + 1; x += 1) {
          for (
               let y = worldY + DUNGEON_ROOM_HEIGHT + 1;
               y >= worldY - 1;
               y -= 1
          ) {
               for (
                    ...
                    if (
                         y >= 0 &&
                         !isSolid(getRegionBlock(region, x, y - 1, z))
                    ) {
                         setRegionBlock(region, x, y, z, BLOCKS.AIR);
                         continue;
                    }
     ...
     for (let chestAttempt = 0; chestAttempt < 2; chestAttempt += 1) {
          for (let tryIndex = 0; tryIndex < 3; tryIndex += 1) {
               const chestX =
                    worldX + random.nextInt(radiusX * 2 + 1) - radiusX;
               const chestZ =
                    worldZ + random.nextInt(radiusZ * 2 + 1) - radiusZ;

               if (
                    getRegionBlock(region, chestX, worldY, chestZ) !==
                    BLOCKS.AIR
               ) {
                    continue;
               }
               ...
               if (
                    isSolid(getRegionBlock(region, chestX - 1, worldY, chestZ))
               ) {
               ...
               if (
                    isSolid(getRegionBlock(region, chestX, worldY, chestZ - 1))
               ) {
                    solidNeighborCount += 1;
               }
               if (
