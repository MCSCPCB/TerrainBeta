...
export function advanceSnowSession(session, columnCount = 1) {
     ...
     for (let localX = session.localX; localX < endX; localX += 1) {
          for (let localZ = 0; localZ < 16; localZ += 1) {
               ...
               const precipitationY = getRegionPrecipitationY(
                    session.region,
                    worldX,
                    worldZ,
               );
               ...
               const adjustedTemperature =
                    session.climate.temperature[climateIndex] -
                    ((precipitationY - 64) / 64.0) * 0.3;
...
export function applySnow(region, climate, chunkX, chunkZ) {
