...
function getTreeCount(treeNoise, random, biomeId, chunkX, chunkZ) {
     ...
     const forestNoise = Math.trunc(
          (noise / 8.0 + random.nextDouble() * 4.0 + 4.0) / 3.0,
     );
     let count = 0;
     ...
     if (
          biomeId === BIOME_IDS.FOREST ||
          biomeId === BIOME_IDS.RAINFOREST ||
          biomeId === BIOME_IDS.TAIGA
     ) {
          count += forestNoise + 5;
     } else if (biomeId === BIOME_IDS.SEASONAL_FOREST) {
          count += forestNoise + 2;
     } else if (
          biomeId === BIOME_IDS.DESERT ||
          biomeId === BIOME_IDS.TUNDRA ||
          biomeId === BIOME_IDS.PLAINS
     ) {
          count -= 20;
     }
