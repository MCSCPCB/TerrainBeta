...
function replaceBiomeSurfaceColumn(state, x) {
     ...
     for (let z = 0; z < 16; z += 1) {
          ...
          const surface = getSurfaceForBiome(
               climateData.biomes[columnIndex],
               surfacePalette,
          );
          const sandy =
               sandField[noiseIndex] + surfaceRandom.nextDouble() * 0.2 > 0.0;
          const gravelly =
               gravelField[noiseIndex] + surfaceRandom.nextDouble() * 0.2 > 3.0;
          ...
          for (let y = 127; y >= 0; y -= 1) {
               ...
               if (remainingDepth === -1) {
                    ...
                    if (y < SEA_LEVEL && topBlock === BLOCKS.AIR) {
                         topBlock =
                              climateData.temperature[columnIndex] < 0.5
                                   ? BLOCKS.ICE
                                   : BLOCKS.WATER;
                    }
                    ...
                    blocks[blockIndex] =
                         y >= SEA_LEVEL - 1 ? topBlock : fillerBlock;
                    continue;
               }
...
export function createSurfaceReplacementSession(
     blocks,
     terrainTables,
     climateData,
     chunkX,
     ...
     const surfaceRandom = new JavaRandom(
          BigInt.asIntN(
               64,
               BigInt(chunkX) * CHUNK_SEED_X + BigInt(chunkZ) * CHUNK_SEED_Z,
          ),
     );
...
export function replaceBlocksForBiomesExact(
     blocks,
     terrainTables,
     climateData,
     chunkX,
