...
const CAVE_REPLACEABLE = new Set([BLOCKS.STONE, BLOCKS.GRASS, BLOCKS.DIRT]);

const OCEAN_BLOCKS = new Set([BLOCKS.WATER]);
...
function caveTunnelMaxSteps(random, range) {
     ...
     return (
          maxBlockRadius -
          random.nextInt(Math.max(1, Math.trunc(maxBlockRadius / 4)))
     );
}
...
function caveSplitStep(random, maxSteps, width) {
     ...
     return (
          Math.trunc(maxSteps / 4) +
          random.nextInt(Math.max(1, Math.trunc(maxSteps / 2)))
     );
}
          ...
          for (let x = minX; x < maxX; x += 1) {
               ...
               while (y >= minY - 1) {
                    if (
                         y >= 0 &&
                         y < 128 &&
                         isOceanBlock(getBlock(blocks, x, y, z))
                    ) {
                         return true;
                    }
...
function carveBlob(
     ...
     const minX = Math.max(
          Math.floor(centerX - radiusH) - targetChunkX * 16 - 1,
          0,
     );
     const maxX = Math.min(
          Math.floor(centerX + radiusH) - targetChunkX * 16 + 1,
     ...
     const minZ = Math.max(
          Math.floor(centerZ - radiusH) - targetChunkZ * 16 - 1,
          0,
     );
     const maxZ = Math.min(
          Math.floor(centerZ + radiusH) - targetChunkZ * 16 + 1,
     ...
     for (let x = minX; x < maxX; x += 1) {
          ...
          for (let z = minZ; z < maxZ; z += 1) {
               ...
               for (let y = maxY; y >= minY; y -= 1) {
                    ...
                    if (
                         normY <= -0.7 ||
                         normXSq + normZSq + normY * normY >= 1.0
                    ) {
                         continue;
                    }
                    ...
                    setBlock(
                         ...
                         y,
                         z,
                         y < 10 ? EXTRA_BLOCKS.FLOWING_LAVA : BLOCKS.AIR,
                    );
                    if (
                         existing === BLOCKS.GRASS &&
                         y > 0 &&
                         getBlock(blocks, x, y - 1, z) === BLOCKS.DIRT
                    ) {
...
function carveTunnel(
     ...
     for (; workingStep < maxSteps; workingStep += 1) {
          const horizontalRadius =
               1.5 + Math.sin((workingStep * Math.PI) / maxSteps) * width;
...
function carveCavesForSourceChunk(
     ...
     const chunkSeed = BigInt.asIntN(
          64,
          (BigInt(sourceChunkX) * xSeed + BigInt(sourceChunkZ) * zSeed) ^
               seed64,
     );
...
export function createCaveCarverSession(
     blocks,
     seed,
     targetChunkX,
     targetChunkZ,
     range = DEFAULT_CAVE_RANGE,
) {
...
export function advanceCaveCarverSession(session, sourceChunkCount = 1) {
     ...
     while (processed < sourceChunkCount && !session.complete) {
          ...
          if (session.sourceChunkZ > session.targetChunkZ + session.range) {
               ...
               if (
                    session.sourceChunkX >
                    session.targetChunkX + session.range
               ) {
...
export function applyCaves(
     ...
     const session = createCaveCarverSession(
          blocks,
          seed,
          targetChunkX,
          targetChunkZ,
