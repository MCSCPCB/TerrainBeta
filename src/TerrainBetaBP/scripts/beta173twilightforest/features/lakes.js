...
export function advanceLakeSession(session) {
  ...
  switch (session.stage) {
    case "water_lake":
      ...
      session.stage = session.sourceBiome === BIOME_IDS.TWILIGHT_SWAMP ? "swamp_lakes" : "lava_lake";
      return true;
    ...
    case "swamp_lakes":
      if (session.swampAttempts < 6) {
        ...
        const lakeY = getRegionSurfaceY(session.region, lakeX, lakeZ) + 1;
        generateLake(session.region, session.random, BLOCKS.WATER, lakeX, lakeY, lakeZ);
    ...
    case "lava_lake":
      if (session.random.nextInt(64) === 0) {
        const lakeX = nextLakeX(session.random, session.chunkX);
        const lakeY = session.random.nextInt(session.random.nextInt(120) + 8);
        const lakeZ = nextLakeZ(session.random, session.chunkZ);
        if (lakeY < 64 || session.random.nextInt(10) === 0) {
          generateLake(session.region, session.random, EXTRA_BLOCKS.LAVA, lakeX, lakeY, lakeZ);
        }
      }
...
export function applyTwilightLakes(region, sourceBiome, random, chunkX, chunkZ) {
  const session = createLakeSession(region, sourceBiome, random, chunkX, chunkZ);
  while (advanceLakeSession(session)) {
    // Keep the same deterministic staged order for synchronous callers.
  }
}
