...
function generateSpring(region, liquidBlock, worldX, worldY, worldZ) {
  ...
  const stoneNeighbors = countHorizontalNeighbors(region, worldX, worldY, worldZ, (blockId) => blockId === BLOCKS.STONE);
  const airNeighbors = countHorizontalNeighbors(region, worldX, worldY, worldZ, (blockId) => blockId === BLOCKS.AIR);
...
export function advanceSpringSession(session, maxAttempts = 1) {
  ...
  session.complete = session.complete || (session.waterAttempt >= 50 && session.lavaAttempt >= 10);
  return processed > 0;
}
