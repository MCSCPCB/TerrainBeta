...
function translate(sx, sy, sz, distance, angle, tilt) {
  ...
  return [
    Math.trunc(sx + Math.round(Math.sin(radiansAngle) * Math.sin(radiansTilt) * distance)),
    Math.trunc(sy + Math.round(Math.cos(radiansTilt) * distance)),
    Math.trunc(sz + Math.round(Math.cos(radiansAngle) * Math.sin(radiansTilt) * distance)),
  ];
}

function drawBresenham(region, x1, y1, z1, x2, y2, z2, blockId, priority = false) {
...
function drawCircle(region, sx, sy, sz, radius, blockId, priority = false) {
  for (let dx = 0; dx <= radius; dx += 1) {
    for (let dz = 0; dz <= radius; dz += 1) {
      let distance = Math.trunc(Math.max(dx, dz) + Math.min(dx, dz) * 0.5);
      if (dx === 3 && dz === 3) {
        distance = 6;
      }
      if (distance > radius) {
...
function placeRoundLeafDisc(region, centerX, centerY, centerZ, radius, leafBlock) {
  const roundedRadius = Math.trunc(radius + 0.618);
  for (let offsetX = -roundedRadius; offsetX <= roundedRadius; offsetX += 1) {
    for (let offsetZ = -roundedRadius; offsetZ <= roundedRadius; offsetZ += 1) {
      const distance = Math.sqrt((Math.abs(offsetX) + 0.5) ** 2 + (Math.abs(offsetZ) + 0.5) ** 2);
      if (distance > radius) {
        continue;
      }

      const existing = getRegionBlock(region, centerX + offsetX, centerY, centerZ + offsetZ);
      if (existing === BLOCKS.AIR || TREE_LEAF_BLOCKS.has(existing)) {
        setRegionBlock(region, centerX + offsetX, centerY, centerZ + offsetZ, leafBlock);
      }
    }
  }
}

function placeTreeFeature(region, random, x, y, z, height, logBlock, leafBlock) {
  let canPlace = true;
  ...
  for (let checkY = y; checkY <= y + 1 + height && canPlace; checkY += 1) {
    ...
    for (let checkX = x - radius; checkX <= x + radius && canPlace; checkX += 1) {
      for (let checkZ = z - radius; checkZ <= z + radius && canPlace; checkZ += 1) {
        ...
        if (!canReplaceTreeBlock(getRegionBlock(region, checkX, checkY, checkZ))) {
          canPlace = false;
        }
      }
    }
  ...
  for (let leafY = y - 3 + height; leafY <= y + height; leafY += 1) {
    ...
    for (let leafX = x - radius; leafX <= x + radius; leafX += 1) {
      ...
      for (let leafZ = z - radius; leafZ <= z + radius; leafZ += 1) {
        ...
        const existing = getRegionBlock(region, leafX, leafY, leafZ);
        if (
          (!isCorner || (random.nextInt(2) !== 0 && layerOffset !== 0))
...
function placePineTree(region, random, x, y, z) {
  ...
  for (let leafY = y + height; leafY >= y + bareTrunkHeight; leafY -= 1) {
    for (let leafX = x - radius; leafX <= x + radius; leafX += 1) {
      ...
      for (let leafZ = z - radius; leafZ <= z + radius; leafZ += 1) {
        ...
        const existing = getRegionBlock(region, leafX, leafY, leafZ);
        if (
          (Math.abs(offsetX) !== radius || Math.abs(offsetZ) !== radius || radius <= 0)
          && !isSolidRenderBlock(existing)
        ) {
          setRegionBlock(region, leafX, leafY, leafZ, EXTRA_BLOCKS.SPRUCE_LEAVES);
        }
      }
    }
  ...
  for (let trunkY = 0; trunkY < height - 1; trunkY += 1) {
    const existing = getRegionBlock(region, x, y + trunkY, z);
    if (existing === BLOCKS.AIR || TREE_LEAF_BLOCKS.has(existing)) {
      setRegionBlock(region, x, y + trunkY, z, EXTRA_BLOCKS.SPRUCE_LOG);
    }
  }
  ...
  for (let offsetY = 0; offsetY <= foliageHeight; offsetY += 1) {
    ...
    for (let leafX = x - radius; leafX <= x + radius; leafX += 1) {
      ...
      for (let leafZ = z - radius; leafZ <= z + radius; leafZ += 1) {
        ...
        const existing = getRegionBlock(region, leafX, leafY, leafZ);
        if (
          (Math.abs(deltaX) !== radius || Math.abs(deltaZ) !== radius || radius <= 0)
          && !isSolidRenderBlock(existing)
        ) {
          setRegionBlock(region, leafX, leafY, leafZ, EXTRA_BLOCKS.SPRUCE_LEAVES);
        }
...
function placeBigOakBranch(region, from, to, logBlock) {
  ...
  for (let offset = 0, limit = delta[majorAxis] + step; offset !== limit; offset += step) {
    ...
    point[minorAxis] = Math.floor(from[minorAxis] + offset * slopeMinor + 0.5);
    point[otherAxis] = Math.floor(from[otherAxis] + offset * slopeOther + 0.5);
...
function placeBigOakTree(region, random, x, y, z) {
  ...
  function getTreeShape(heightOffset) {
    ...
    if (distance === 0.0) {
      ...
      radius = Math.sqrt(Math.abs(center) ** 2 - Math.abs(distance) ** 2);
    }
    return radius * 0.5;
  }
  ...
  function getClusterShape(layer) {
    ...
    return layer !== 0 && layer !== config.foliageClusterHeight - 1 ? 3.0 : 2.0;
  }

  function placeFoliageCluster(clusterX, baseY, clusterZ) {
    for (let layerY = baseY; layerY < baseY + config.foliageClusterHeight; layerY += 1) {
      const shape = getClusterShape(layerY - baseY);
      placeRoundLeafDisc(region, clusterX, layerY, clusterZ, shape, EXTRA_BLOCKS.OAK_LEAVES);
    }
  }
  ...
  function canPlace() {
    ...
    const top = [config.worldX, config.worldY + config.height - 1, config.worldZ];
    const soil = getRegionBlock(region, config.worldX, config.worldY - 1, config.worldZ);
  ...
  function makeBranches() {
    ...
    let branchCount = Math.trunc(1.382 + ((config.foliageDensity * config.height) / 13.0) ** 2);
    if (branchCount < 1) {
      branchCount = 1;
    }

    const branchData = new Array(branchCount * config.height);
    let clusterY = config.worldY + config.height - config.foliageClusterHeight;
    let clusterCount = 1;
    ...
    while (heightOffset >= 0) {
      ...
      for (let branchIndex = 0; branchIndex < branchCount; branchIndex += 1) {
        const branchLength = config.branchLengthScale * (treeShape * (featureRandom.nextFloat() + 0.328));
        const angle = featureRandom.nextFloat() * 2.0 * 3.14159;
        const branchX = Math.floor(branchLength * Math.sin(angle) + config.worldX + 0.5);
        const branchZ = Math.floor(branchLength * Math.cos(angle) + config.worldZ + 0.5);
        const foliageBase = [branchX, clusterY, branchZ];
        const foliageTop = [branchX, clusterY + config.foliageClusterHeight, branchZ];

        if (bigOakTryBranch(region, foliageBase, foliageTop) !== -1) {
          continue;
        }

        const trunkBase = [config.worldX, config.worldY, config.worldZ];
        const distance = Math.sqrt(
          Math.abs(config.worldX - foliageBase[0]) ** 2 + Math.abs(config.worldZ - foliageBase[2]) ** 2,
        );
        ...
        if (bigOakTryBranch(region, trunkBase, foliageBase) !== -1) {
          continue;
        }

        branchData[clusterCount] = [branchX, clusterY, branchZ, trunkBase[1]];
        clusterCount += 1;
      }
  ...
  setRegionBlock(region, config.worldX, config.worldY - 1, config.worldZ, BLOCKS.DIRT);
  placeBigOakBranch(
    region,
...
function placeCanopyTree(region, random, x, y, z) {
  ...
  const buildBranch = (height, length, angle, tilt) => {
    ...
    drawBresenham(region, src[0], src[1], src[2], dest[0], dest[1], dest[2], EXTRA_BLOCKS.SPRUCE_LOG, true);
    putBlock(region, dest[0] + 1, dest[1], dest[2], EXTRA_BLOCKS.SPRUCE_LOG, true);
    putBlock(region, dest[0] - 1, dest[1], dest[2], EXTRA_BLOCKS.SPRUCE_LOG, true);
    putBlock(region, dest[0], dest[1], dest[2] + 1, EXTRA_BLOCKS.SPRUCE_LOG, true);
    putBlock(region, dest[0], dest[1], dest[2] - 1, EXTRA_BLOCKS.SPRUCE_LOG, true);
    drawCircle(region, dest[0], dest[1] - 1, dest[2], 3, EXTRA_BLOCKS.SPRUCE_LEAVES, false);
    drawCircle(region, dest[0], dest[1], dest[2], 4, EXTRA_BLOCKS.SPRUCE_LEAVES, false);
    drawCircle(region, dest[0], dest[1] + 1, dest[2], 2, EXTRA_BLOCKS.SPRUCE_LEAVES, false);
  };
...
function placeMangroveTree(region, random, x, y, z) {
  ...
  const buildBranch = (height, length, angle, tilt) => {
    const src = [x, y + height, z];
    const dest = translate(src[0], src[1], src[2], length, angle, tilt);
    drawBresenham(region, src[0], src[1], src[2], dest[0], dest[1], dest[2], EXTRA_BLOCKS.OAK_LOG, true);
    putBlock(region, dest[0] + 1, dest[1], dest[2], EXTRA_BLOCKS.OAK_LOG, true);
    putBlock(region, dest[0] - 1, dest[1], dest[2], EXTRA_BLOCKS.OAK_LOG, true);
    putBlock(region, dest[0], dest[1], dest[2] + 1, EXTRA_BLOCKS.OAK_LOG, true);
    putBlock(region, dest[0], dest[1], dest[2] - 1, EXTRA_BLOCKS.OAK_LOG, true);
    const branchSize = 2 + random.nextInt(3);
    drawCircle(region, dest[0], dest[1] - 1, dest[2], branchSize - 1, EXTRA_BLOCKS.BIRCH_LEAVES, false);
    drawCircle(region, dest[0], dest[1], dest[2], branchSize, EXTRA_BLOCKS.BIRCH_LEAVES, false);
    drawCircle(region, dest[0], dest[1] + 1, dest[2], branchSize - 2, EXTRA_BLOCKS.BIRCH_LEAVES, false);
  };
  ...
  for (let branch = 0; branch < branchCount; branch += 1) {
    buildBranch(7 + branch, 6 + random.nextInt(2), 0.3 * branch + offset, 0.25);
  }

  const rootCount = 3 + random.nextInt(2);
  offset = random.nextDouble();
  for (let root = 0; root < rootCount; root += 1) {
    buildRoot(5, 8.0, 0.4 * root + offset, 0.75 + random.nextDouble() * 0.1);
  }
...
function placeSelectedTwilightTree(region, random, sourceBiome, x, y, z) {
  switch (selectTwilightTreeType(sourceBiome, random)) {
    ...
    case "birch":
      return placeTreeFeature(region, random, x, y, z, random.nextInt(3) + 5, EXTRA_BLOCKS.BIRCH_LOG, EXTRA_BLOCKS.BIRCH_LEAVES);
    case "pine":
    ...
    default:
      return placeTreeFeature(region, random, x, y, z, random.nextInt(3) + 4, EXTRA_BLOCKS.OAK_LOG, EXTRA_BLOCKS.OAK_LEAVES);
  }
}

function getTwilightTreeCount(forestNoise, random, sourceBiome, chunkX, chunkZ) {
  const mapX = chunkX * 16;
  const mapZ = chunkZ * 16;
  const forestNoiseValue = forestNoise.generateNoiseForCoordinate(mapX * 0.5, mapZ * 0.5);
  const baseCount = Math.trunc((forestNoiseValue / 8.0 + random.nextDouble() * 4.0 + 4.0) / 3.0);
...
export function createTreeSession(region, sourceBiome, forestNoise, random, chunkX, chunkZ) {
  return {
    region,
    sourceBiome,
    forestNoise,
...
export function advanceTreeSession(session, maxAttempts = 1) {
  ...
  while (processed < maxAttempts && session.attempt < session.treeCount) {
    ...
    if (baseY >= 1 && baseY < 128) {
      placeSelectedTwilightTree(session.region, session.random, session.sourceBiome, x, baseY, z);
    }
...
export function applyTwilightTrees(region, sourceBiome, forestNoise, random, chunkX, chunkZ) {
  const session = createTreeSession(region, sourceBiome, forestNoise, random, chunkX, chunkZ);
  while (advanceTreeSession(session)) {
    // Preserve the exact attempt ordering for synchronous callers.
  }
}
