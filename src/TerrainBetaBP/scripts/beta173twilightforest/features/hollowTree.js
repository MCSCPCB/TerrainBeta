...
import { TREE_LEAF_BLOCKS, getRegionBlock } from "../population/shared.js";
import {
     drawBlob,
     drawBresenham,
     putBlock,
...
function canGenerateHollowTree(
     region,
     worldX,
     worldY,
     worldZ,
     ...
     for (let offsetX = -crownRadius; offsetX <= crownRadius; offsetX += 1) {
          for (
               let offsetZ = -crownRadius;
               offsetZ <= crownRadius;
               offsetZ += 1
          ) {
               for (
                    let offsetY = height - crownRadius;
                    offsetY <= height + crownRadius;
                    offsetY += 1
               ) {
                    const blockId = getRegionBlock(
                         region,
...
export function generateHollowTree(region, random, worldX, worldY, worldZ) {
     ...
     function makeLargeBranchFrom(
          ...
          const numMedBranches =
               state.random.nextInt(medBranchBase) +
     ...
     function buildBranchRing(
          ...
          const branchCount =
               state.random.nextInt(Math.max(1, maxBranches - minBranches)) +
               minBranches;
          ...
          for (let index = 0; index <= branchCount; index += 1) {
               const variedHeight =
                    heightVariation > 0
                         ? branchHeight -
                           heightVariation +
                           state.random.nextInt(2 * heightVariation)
                         : branchHeight;
     ...
     function buildFullCrown() {
          ...
          buildBranchRing(
               state.height - Math.trunc(crownRadius / 2),
               0,
               crownRadius,
               0,
               0.28,
     ...
     function buildTrunk() {
          ...
          for (
               let offsetX = -state.diameter;
               offsetX <= state.diameter;
               offsetX += 1
          ) {
               for (
                    let offsetZ = -state.diameter;
                    offsetZ <= state.diameter;
                    offsetZ += 1
               ) {
                    const distance = Math.trunc(
                         Math.max(Math.abs(offsetX), Math.abs(offsetZ)) +
                              Math.min(Math.abs(offsetX), Math.abs(offsetZ)) *
                                   0.5,
                    );
     ...
     for (let index = 0; index <= numBranches; index += 1) {
          const branchHeight =
               Math.trunc(state.height * state.random.nextDouble() * 0.9) +
               Math.trunc(state.height / 10);
          makeSmallBranch(
               branchHeight,
               4.0,
               state.random.nextDouble(),
...
export function advanceHollowTreeSession(session) {
     ...
     if (session.random.nextInt(4) === 0) {
          ...
          while (
               worldY > 0 &&
               getRegionBlock(session.region, worldX, worldY, worldZ) ===
                    BLOCKS.AIR
          ) {
               worldY -= 1;
          }
