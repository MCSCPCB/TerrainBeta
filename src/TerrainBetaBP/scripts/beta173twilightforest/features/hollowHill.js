...
function makeSpike(region, random, blockId, worldX, worldY, worldZ, length, direction) {
  ...
  for (let offsetX = -width; offsetX <= width; offsetX += 1) {
    for (let offsetZ = -width; offsetZ <= width; offsetZ += 1) {
      ...
      if (distance === 0) {
        ...
        spikeLength = random.nextInt(Math.max(1, Math.trunc(length / (distance + 0.25))));
      }

      for (let offsetY = 0; offsetY !== spikeLength * direction; offsetY += direction) {
        putBlock(region, worldX + offsetX, worldY + offsetY, worldZ + offsetZ, blockId, false);
      }
    }
  }
...
function generateCaveStalactite(region, random, blockId, sizeFactor, hang, worldX, worldY, worldZ) {
  let ceiling = 129;
...
function makeRandomOreStalactite(random, caveSize) {
  if (caveSize >= 3) {
    ...
    if (selector === 0) {
      return { blockId: EXTRA_BLOCKS.DIAMOND_ORE, sizeFactor: random.nextDouble() * 0.5, hang: true };
    }
    if (selector === 1) {
      return { blockId: EXTRA_BLOCKS.GOLD_ORE, sizeFactor: random.nextDouble() * 0.8, hang: true };
    }
  }
  ...
  if (caveSize >= 2) {
    const selector = random.nextInt(6);
    if (selector === 0) {
      return { blockId: EXTRA_BLOCKS.REDSTONE_ORE, sizeFactor: random.nextDouble() * 0.6, hang: true };
    }
    if (selector === 1 || selector === 2) {
      return { blockId: EXTRA_BLOCKS.LAPIS_ORE, sizeFactor: random.nextDouble() * 0.8, hang: true };
    }
  }

  const selector = random.nextInt(5);
  if (selector === 0 || selector === 1) {
    return { blockId: EXTRA_BLOCKS.IRON_ORE, sizeFactor: random.nextDouble() * 0.7, hang: true };
  }
  if (selector === 2 || selector === 3) {
    return { blockId: EXTRA_BLOCKS.COAL_ORE, sizeFactor: random.nextDouble() * 0.8, hang: true };
  }
  return { blockId: EXTRA_BLOCKS.GLOWSTONE, sizeFactor: random.nextDouble() * 0.5, hang: true };
}
...
export function createHollowHillSession(region, random, size, worldX, worldY, worldZ) {
...
export function advanceHollowHillSession(session, maxAttempts = 1) {
  ...
  while (processed < maxAttempts && !session.complete) {
    const [targetX, targetZ] = getCoordsInHill2D(session.random, session.worldX, session.worldZ, session.radius);

    switch (session.stage) {
      case "spawners":
        putBlock(session.region, targetX, session.worldY + session.random.nextInt(4), targetZ, EXTRA_BLOCKS.MOB_SPAWNER, true);
        break;

      case "treasure":
        putBlock(session.region, targetX, session.worldY, targetZ, EXTRA_BLOCKS.CHEST, true);
        break;

      case "ore_stalactites": {
        const stalactite = makeRandomOreStalactite(session.random, session.size);
    ...
    const limit = session.stage === "spawners"
...
export function applyTwilightHollowHill(region, random, size, worldX, worldY, worldZ) {
  const session = createHollowHillSession(region, random, size, worldX, worldY, worldZ);
  while (advanceHollowHillSession(session)) {
    // Preserve deterministic stage ordering for synchronous callers.
  }
}
