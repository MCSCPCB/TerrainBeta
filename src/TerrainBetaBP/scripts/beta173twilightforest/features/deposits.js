...
const ORE_CONFIG = Object.freeze([
  { block: BLOCKS.DIRT, attempts: 20, size: 32 },
  { block: BLOCKS.GRAVEL, attempts: 10, size: 32 },
  { block: EXTRA_BLOCKS.COAL_ORE, attempts: 20, size: 16 },
  { block: EXTRA_BLOCKS.IRON_ORE, attempts: 20, size: 8 },
  { block: EXTRA_BLOCKS.GOLD_ORE, attempts: 1, size: 6 },
  { block: EXTRA_BLOCKS.DIAMOND_ORE, attempts: 1, size: 6 },
]);

function pickOreY(random, configIndex) {
  switch (configIndex) {
    case 0:
    case 1:
    case 2:
    ...
    case 4:
      return random.nextInt(16) + random.nextInt(16);
    default:
      return random.nextInt(16) + random.nextInt(16);
  }
}
...
function generateReplaceableVein(region, random, replaceableBlocks, oreBlock, size, baseX, baseY, baseZ) {
  ...
  for (let step = 0; step <= size; step += 1) {
    ...
    const maxX = Math.min(Math.floor(centerX + radius), region.originWorldX + POPULATION_REGION_SIZE - 1);
    ...
    const maxZ = Math.min(Math.floor(centerZ + radius), region.originWorldZ + POPULATION_REGION_SIZE - 1);
    ...
    for (let x = minX; x <= maxX; x += 1) {
      ...
      for (let y = minY; y <= maxY; y += 1) {
        ...
        if (dxSquared + dySquared >= 1.0) {
          continue;
        }

        for (let z = minZ; z <= maxZ; z += 1) {
          const dz = ((z + 0.5) - centerZ) / radius;
          if (dxSquared + dySquared + dz * dz >= 1.0) {
            continue;
          }
...
export function advanceOreSession(session, maxAttempts = 1) {
  ...
  while (processed < maxAttempts && session.configIndex < ORE_CONFIG.length) {
    ...
    const y = pickOreY(session.random, session.configIndex);
...
export function applyTwilightOres(region, random, chunkX, chunkZ) {
  const session = createOreSession(region, random, chunkX, chunkZ);
  while (advanceOreSession(session)) {
    // Keep the same deterministic state progression for synchronous callers.
  }
}
